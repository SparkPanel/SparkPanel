...
use SparkPanel\Exceptions\Http\Server\ServerStateConflictException;

/**
 * \SparkPanel\Models\Server.
 *
 * @property int $id
 ...
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\ActivityLog[] $activity
 * @property int|null $activity_count
 * @property Allocation|null $allocation
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Allocation[] $allocations
 * @property int|null $allocations_count
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Backup[] $backups
 * @property int|null $backups_count
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Database[] $databases
 * @property int|null $databases_count
 * @property Egg|null $egg
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Mount[] $mounts
 * @property int|null $mounts_count
 * @property Nest $nest
 * @property Node $node
 * @property \Illuminate\Notifications\DatabaseNotificationCollection|\Illuminate\Notifications\DatabaseNotification[] $notifications
 * @property int|null $notifications_count
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Schedule[] $schedules
 * @property int|null $schedules_count
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\Subuser[] $subusers
 * @property int|null $subusers_count
 * @property ServerTransfer|null $transfer
 * @property User $user
 * @property \Illuminate\Database\Eloquent\Collection|\SparkPanel\Models\EggVariable[] $variables
...
class Server extends Model
{
    ...
    public static function validationRules(array $data = []): array
    {
        return [
            ...
            'description' => 'nullable|string|min:3|max:400',
            'owner_id' => 'required|integer|exists:users,id',
            'node_id' => 'required|integer|exists:nodes,id',
            'memory' => 'required|integer|min:0',
            'swap' => 'required|integer|min:-1',
            'disk' => 'required|integer|min:0',
            'io' => 'required|integer|between:0,10',
            'cpu' => 'required|integer|min:0',
            'threads' => 'nullable|string|regex:/^([0-9,]+)$/u',
            ...
            'image' => 'required|string|regex:/^[a-zA-Z0-9\-_:\/\.]+$/u',
    ...
    public function variables(): HasMany
    {
        return $this->hasMany(EggVariable::class, 'egg_id', 'egg_id')
            ...
            ->leftJoin('server_variables', function (JoinClause $join) {
                ...
                //
                // @see https://github.com/SparkPanel/panel/issues/2250
    ...
    {
        if ($this->isSuspended()) {
            throw new ServerStateConflictException($this, 'Server is suspended');
        }

        if ($this->node->isUnderMaintenance()) {
            throw new ServerStateConflictException($this, 'Node is under maintenance');
        }

        if (!$this->isInstalled()) {
            throw new ServerStateConflictException($this, 'Server is not installed');
        }

        if ($this->status === self::STATUS_RESTORING_BACKUP) {
            throw new ServerStateConflictException($this, 'Server is restoring a backup');
        }

        if (!is_null($this->transfer)) {
            throw new ServerStateConflictException($this, 'Server is being transferred');
        }
    }
    ...
    public function validateTransferState()
    {
        ...
        if (!is_null($this->transfer)) {
            throw new ServerStateConflictException($this, 'Server is already being transferred');
        }
    }
}
