...
export async function registerRoutes(app: Express): Promise<HTTPServer> {
  ...
  statsInterval = setInterval(async () => {
    try {
      ...
      console.error("Stats update error:", error instanceof Error ? error.message : error);
    }
...
async function setupSftpUserInContainer(
  ...
  try {
    ...
    await new Promise<void>((resolve) => {
      ...
      removeStream.on("end", () => { cleanup(); resolve(); });
      removeStream.on("error", () => { cleanup(); resolve(); });
      timeoutId = setTimeout(() => { cleanup(); resolve(); }, 5000);
    });
...
async function removeSftpUserFromContainer(
  ...
  try {
    ...
    const sshRemoveStream = await sshConfigRemoveExec.start({ hijack: true, stdin: false });
    await new Promise<void>((resolve) => {
      let timeoutId: NodeJS.Timeout | null = null;
      const cleanup = () => { if (timeoutId) clearTimeout(timeoutId); };
      sshRemoveStream.on("end", () => { cleanup(); resolve(); });
      sshRemoveStream.on("error", () => { cleanup(); resolve(); });
      timeoutId = setTimeout(() => { cleanup(); resolve(); }, 5000);
    });
