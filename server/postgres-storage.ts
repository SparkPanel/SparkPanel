...
export class PostgresStorage implements IStorage {
  ...
  private panelSettings: {
    ...
    panelName: "SparkPanel beta 1.0.0",
  ...
  private settingsFilePath(): string {
    return join(process.cwd(), "panel-settings.json");
  }
  ...
  private async loadPanelSettings(): Promise<void> {
    try {
      ...
      this.panelSettings = {
        panelName: saved.panelName ?? "SparkPanel beta 1.0.0",
        primaryColor: saved.primaryColor,
        backgroundColor: saved.backgroundColor,
        borderColor: saved.borderColor,
        sidebarAccentColor: saved.sidebarAccentColor ?? "#e5e7eb",
      };
    } catch {
      this.panelSettings = {
        panelName: "SparkPanel beta 1.0.0",
  ...
  private async savePanelSettings(): Promise<void> {
    try {
      await writeFile(this.settingsFilePath(), JSON.stringify(this.panelSettings, null, 2), "utf-8");
    } catch (err) {
      console.error("Failed to persist panel settings:", err);
    }
  }
