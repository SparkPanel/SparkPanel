...
export class VdsTerminal {
  ...
  async connect(userId: string, ws: WebSocket): Promise<void> {
    try {
      ...
      // Проверяем права доступа (только админы могут использовать VDS терминал)
      if (user.role !== "admin") {
        ws.send(JSON.stringify({
          type: "vds_terminal_error",
          message: "Access denied. Only administrators can use VDS terminal.",
        }));
        return;
      }
  ...
  async executeCommand(userId: string, command: string, ws: WebSocket): Promise<void> {
    try {
      ...
      if (user.role !== "admin") {
        ws.send(JSON.stringify({
          type: "vds_terminal_error",
          message: "Access denied. Only administrators can execute commands.",
        }));
        return;
      }
