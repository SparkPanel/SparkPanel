...
namespace SparkPanel\Services\Helpers;

use GuzzleHttp\Client;
use Carbon\CarbonImmutable;
use Illuminate\Support\Arr;
use Illuminate\Contracts\Cache\Repository as CacheRepository;
use SparkPanel\Exceptions\Service\Helper\CdnVersionFetchingException;

class SoftwareVersionService
{
    public const VERSION_CACHE_KEY = 'SparkPanel:versioning_data';
    ...
    {
        return Arr::get(self::$result, 'discord') ?? 'https://SparkPanel.io/discord';
    }
    ...
    {
        return $this->cache->remember(self::VERSION_CACHE_KEY, CarbonImmutable::now()->addMinutes(config('SparkPanel.cdn.cache_time', 60)), function () {
            try {
                $response = $this->client->request('GET', config('SparkPanel.cdn.url'));
